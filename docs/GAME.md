...
- The car card (`carCard` in page.js) — the day's `sprite` drawn again at 6× (a 96×144 canvas shown at 96 CSS px, `image-rendering: pixelated`) at the top of the game card, over a radial accent glow. It calls the same `buildSprite` the game does, so there is one drawing of the car and no new day-file field; a sprite that is missing or has no rows falls back to `DEFAULT_SPRITE`, and anything that throws leaves the card out entirely rather than taking the page down. The canvas carries `role="img"` and an `aria-label` naming the car, since the picture is the content.
